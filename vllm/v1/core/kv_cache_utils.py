...
def _promote_indivisible_block_sizes(
    ...
    for name, spec in candidates.items():
        ...
        if primary_block is not None and primary_block % new_block_size != 0:
            # Both constraints must hold: divide the primary block (scheduler
            # LCM invariant, kv_cache_coordinator asserts scheduler %
            # group_block == 0) AND stay a whole multiple of this layer's own
            # kernel granularity (spec.block_size, 16 for the SW drafter).
            # Upstream unify scales by whole ratios for the same reason; a
            # divisor like 460/920 of 1840 satisfies the first but breaks the
            # second (460 % 16 == 12, 920 % 16 == 8) and dies downstream or
            # mis-aligns KV pages.
            # spec.block_size is the layer's *current framework* block, not
            # the backend's get_supported_kernel_block_sizes(). It is the
            # kernel granularity here only because nothing has promoted this
            # layer yet; for a layer whose block was already promoted the two
            # diverge, and this condition would then be the weaker of the two.
            divisor = next(
                (
                    d
                    for d in range(new_block_size, primary_block + 1)
                    if primary_block % d == 0 and d % spec.block_size == 0
                ),
                None,
            )
            if divisor is None:
